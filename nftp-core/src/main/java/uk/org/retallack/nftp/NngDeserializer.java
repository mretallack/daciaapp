...
public class NngDeserializer {
     ...
     * Strategy: try VLU-length first. If the decoded length is valid and the
     * string contains no null bytes, accept it. Otherwise fall back to null-terminated.
     */
    private String readStringAuto() {
        int savedPos = pos;
        // Try VLU-length-prefixed first
        int len = (int) readVlu();
        if (len >= 0 && pos + len <= data.length) {
            // Check the region doesn't contain a premature null that would indicate
            // this is actually null-terminated with the "length" being the first char
            boolean hasNull = false;
            for (int i = pos; i < pos + len; i++) {
                if (data[i] == 0) { hasNull = true; break; }
            }
            if (!hasNull) {
                String s = new String(data, pos, len, StandardCharsets.UTF_8);
                pos += len;
        ...
        // VLU length was bogus, fall back to null-terminated
        pos = savedPos;
        return readNullTerminatedString();
    }
